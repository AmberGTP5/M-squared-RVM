...
model = AutoModelForCausalLM.from_pretrained('/root/autodl-tmp/multimodal/train_multimodal_from_scratch/save/geo_sft')
model.to(device)
q_text = tokenizer.apply_chat_template([{"role":"system", "content":'You are a helpful assistant.'}, {"role":"user", "content":'请详细描述图片内容\n<image>'}], \
            tokenize=False, \
            add_generation_prompt=True).replace('<image>', '<|image_pad|>'*243)

input_ids = tokenizer(q_text, return_tensors='pt')['input_ids']
input_ids = input_ids.to(device)
image = Image.open('/root/autodl-tmp/multimodal/dataset/Geo170K/images/test/3.png').convert("RGB")
